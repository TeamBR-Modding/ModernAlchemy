...
public class UpdateServerSuperCoilLists implements IMessageHandler<UpdateServerSuperCoilLists.UpdateMessage, IMessage> {

    @Override
    public IMessage onMessage(UpdateServerSuperCoilLists.UpdateMessage message, MessageContext ctx) {
        if (ctx.side.isServer()) {
            TileSuperTeslaCoil tile = (TileSuperTeslaCoil) ctx.getServerHandler().playerEntity.worldObj.getTileEntity(message.x, message.y, message.z);
            ...
            TileSuperTeslaCoil tile = (TileSuperTeslaCoil) Minecraft.getMinecraft().theWorld.getTileEntity(message.x, message.y, message.z);
            if (message.listName.equals("linkedMachines")) {
                tile.linkedMachines.clear();
                tile.linkedMachines.addAll(message.list);
            } else if (message.listName.equals("rangeMachines")) {
